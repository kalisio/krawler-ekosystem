...
# Sub-package in the monorepo from which the app image is built.
# kash's init_lib_infos reads "$PKG_DIR/package.json" to derive name/version.
PKG_DIR="$ROOT_DIR/packages/krawler"
...
## Init package infos
##
# NOTE: passing PKG_DIR (not ROOT_DIR) so kash reads packages/krawler/package.json.
# get_git_tag / get_git_branch still operate on the git repo (they resolve upwards).
init_lib_infos "$PKG_DIR"
...
DOCKER_BUILDKIT=1 docker build -f "$PKG_DIR/dockerfile" \
