---
title: Hooks
description: The built-in processing functions
outline: [2, 3]
---
...
::: tip
Although all are not applicable, [common FeathersJS hooks](https://hooks-common.feathersjs.com/) are exposed in addition to Krawler hooks and can be used in jobs, e.g. you can add `disallow: 'external'` to avoid exposing some services when deploying as a web app.
:::
...
[source](https://github.com/kalisio/krawler-ekosystem/blob/master/packages/krawler/src/hooks/hooks.auth.js)
...
[source](https://github.com/kalisio/krawler-ekosystem/blob/master/packages/krawler/src/hooks/hooks.clear.js)
...
[source](https://github.com/kalisio/krawler-ekosystem/blob/master/packages/krawler/src/hooks/hooks.csv.js)
...
[source](https://github.com/kalisio/krawler-ekosystem/blob/master/packages/krawler/src/hooks/hooks.ftp.js)
...
[source](https://github.com/kalisio/krawler-ekosystem/blob/master/packages/krawler/src/hooks/hooks.grid.js)
...
Split input gridded data into a set of tiles (a tileset), each tile carrying its data subset and a GeoJSON geometry covering its bounds. It is typically used after [resampleGrid](#resamplegrid-options) to produce per-tile features ready to be written or pushed to a database. Hook options are the following:
* **input**: input grid specification, same structure as [resampleGrid](#resamplegrid-options) (`bounds`, `origin`, `size`, `resolution`)
* **output**: output specification
  * **resolution**: the tile resolution as an array of decimal values `[longitude resolution, latitude resolution]`
* **dataPath**: property path where to read the input grid data and write the resulting tiles on the hook object, defaults to `result.data`
* **transform**: perform transformation using these options before write, see description in [transformJson](#transformjson-options)

## IMAP

[source](https://github.com/kalisio/krawler-ekosystem/blob/master/packages/krawler/src/hooks/hooks.imap.js)
...
Connect to an **IMAP** server. The [connection options](https://imapflow.com/module-imapflow-ImapFlow.html) of the client are defined in the hook options plus:
...
[source](https://github.com/kalisio/krawler-ekosystem/blob/master/packages/krawler/src/hooks/hooks.json.js)
...
[source](https://github.com/kalisio/krawler-ekosystem/blob/master/packages/krawler/src/hooks/hooks.geojson.js)

### readGeoJson(options)

Read a GeoJSON from an input stream/store and convert it to in-memory JSON values. This is a convenience alias of [readJson](#readjson-options) and accepts exactly the same options.

### writeGeoJson(options)

Generate a GeoJSON file from in-memory JSON values. This is a convenience alias of [writeJson](#writejson-options) and accepts exactly the same options.
...
* **geometry**: property path where to read an already-built GeoJSON geometry object, defaults to `geometry`; when present on an object it is used as-is instead of building a `Point` from longitude/latitude/altitude
...
[source](https://github.com/kalisio/krawler-ekosystem/blob/master/packages/krawler/src/hooks/hooks.kml.js)
...
[source](https://github.com/kalisio/krawler-ekosystem/blob/master/packages/krawler/src/hooks/hooks.logger.js)

These hooks let you attach a [Winston](https://github.com/winstonjs/winston) logger to a job/task and write custom log entries from your pipeline.

### createLogger(options)

Create and attach a Winston logger to the hook item. Hook options are the following:
* **loggerPath**: property path where to store the created logger on the hook item, defaults to `logger`
* **level**: the logger level (defaults to `debug` when `NODE_ENV=development`, `info` otherwise)
* **format**: a Winston format to apply to the logger
* any other key is treated as a Winston [transport](https://github.com/winstonjs/winston/blob/master/docs/transports.md) name (e.g. `Console`, `File`, `MongoDB`) whose value is the transport options object; a transport option can include an `import` property naming a module to dynamically import first (e.g. a Winston plugin like `winston-mongodb`)

::: tip
Winston and its plugins are imported dynamically so that the job's own Winston instance (with its plugins) is used rather than Krawler's. By default a colorized `Console` transport is configured.
:::

### removeLogger(options)

Detach the logger from the hook item. Hook options are the following:
* **loggerPath**: property path where to retrieve the logger on the hook item, defaults to `logger`

### log(options)

Write a log entry using a user-defined function. Hook options are the following:
* **function**: a function taking the logger and the hook item as input — `(logger, item) => { ... }` (can be async); as a shortcut `options` can directly be this function
* **loggerPath**: property path where to retrieve the logger on the hook item, defaults to `logger`

::: warning
The logger must have been set up with [createLogger](#createlogger-options) before using this hook.
:::

## MongoDB

[source](https://github.com/kalisio/krawler-ekosystem/blob/master/packages/krawler/src/hooks/hooks.mongo.js)

### connectMongo(options)

Connect to a MongoDB database. The [connection options](https://mongodb.github.io/node-mongodb-native/) of the client are defined in the hook options plus:
...
::: tip
**Krawler** relies on the official [node-mongodb-native](https://github.com/mongodb/node-mongodb-native) driver. Any remaining hook option is passed as-is to `MongoClient.connect()`.
:::
...
### deleteMongoBucket(options)

Delete a file from an existing bucket. Hook options are the following:
* **bucket**: the name of the bucket to delete the file from, defaults to the hook object ID
* **key**: the file name to delete, can be a template with item as context, defaults to the hook object ID

> The hook throws if the file cannot be found in the bucket.

## Feathers

[source](https://github.com/kalisio/krawler-ekosystem/blob/master/packages/krawler/src/hooks/hooks.feathers.js)
...
[source](https://github.com/kalisio/krawler-ekosystem/blob/master/packages/krawler/src/hooks/hooks.nwp.js)
...
[source](https://github.com/kalisio/krawler-ekosystem/blob/master/packages/krawler/src/hooks/hooks.ogc.js)

### getCapabilities(options)

Execute a `GetCapabilities` request to get the general information about an OGC service such as WMS, WCS, WPS... The response is parsed from XML to an in-memory JSON object. Hook options are the following:
* **url**: the base URL of the request to be executed
* **headers**: an object of HTTP headers to send with the request (e.g. to pass an access token)
* **dataPath**: property path where to store the resulting JSON object on the hook object, defaults to `result.data`
* any other option (e.g. **service**, **version**) is appended as a query parameter of the request (`request=GetCapabilities` is always added)
...
[source](https://github.com/kalisio/krawler-ekosystem/blob/master/packages/krawler/src/hooks/hooks.pg.js)
...
[source](https://github.com/kalisio/krawler-ekosystem/blob/master/packages/krawler/src/hooks/hooks.raster.js)
...
[source](https://github.com/kalisio/krawler-ekosystem/blob/master/packages/krawler/src/hooks/hooks.store.js)
...
[source](https://github.com/kalisio/krawler-ekosystem/blob/master/packages/krawler/src/hooks/hooks.system.js)
...
[source](https://github.com/kalisio/krawler-ekosystem/blob/master/packages/krawler/src/hooks/hooks.txt.js)
...
[source](https://github.com/kalisio/krawler-ekosystem/blob/master/packages/krawler/src/hooks/hooks.utils.js)
...
> Tasks and write hooks automatically track generated outputs but sometimes outputs are generated by an external process (eg. command hook) so that you need to declare it in order to properly clean it with the [clearOutputs](#clearoutputs-options) hook.
...
[source](https://github.com/kalisio/krawler-ekosystem/blob/master/packages/krawler/src/hooks/hooks.xml.js)
...
[source](https://github.com/kalisio/krawler-ekosystem/blob/master/packages/krawler/src/hooks/hooks.yaml.js)
