...
function validateFeature (feature) {
  ...
  if (feature.properties.NivInfViCr < 1) {
    console.warn(`Value for ${feature.properties.name} is invalid: ${feature.properties.NivInfViCr}. Setting the value to '1'`)
    _.set(feature, 'properties.NivInfViCr', 1)
  }
  if (feature.properties.NivInfViCr > 4) {
    console.warn(`Value for ${feature.properties.name} is invalid: ${feature.properties.NivInfViCr}. Setting the value to '4'`)
  ...
  if (getType(feature) === 'MultiLineString') {
    ...
    // Rebuild geometry from the clean lines
      ...
      after: {
        ...
        generateForecasts: {
          ...
          function: (item) => {
            ...
            const features = _.get(item, 'data.features', [])
            _.forEach(features, feature => {
              const forecastFeature = envelope(feature)
              ...
              _.set(forecastFeature, 'properties.NivSituVigiCruEnt', feature.properties.NivInfViCr) // backward compatibility
