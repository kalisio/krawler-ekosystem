...
async function lftpCommand (client, commands, settings = {}) {
  ...
  if (settings) {
    ...
    _.forOwn(settings, (value, key) => {
      settingsCommands.push(`set ${key} ${value}`)
    })
    commands = settingsCommands.concat(commands)
  }
...
export function getFTP (options = {}) {
  async function get (item, hook) {
    ...
    const localFilePath = path.join(store.path, localFile)
    ...
    if (fs.existsSync(localFilePath)) {
      debug('Deletting local file ' + localFilePath)
