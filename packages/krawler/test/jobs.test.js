  ...
  it('creates the job store', () => {
    ...
    return storesService.create({
      ...
      .then(store => {
        ...
        expect(store.path).toBe(outputPath)
        storage = store
      })
  })
  ...
  it('creates a job retrying to success after a failed HTTP 403 status task', () => {
    ...
    return jobsService.create({
      ...
      .then(tasks => {
        expect(tasks).toBeTruthy()
        expect(tasks.length).toBe(1)
        expect(tasks[0].options.url).toBe('https://www.google.fr')
      })
  }, 5000)
  ...
  it('creates a job with task hooks', () => {
    return jobsService.create({
      ...
      .then(tasks => {
        expect(tasks).toBeTruthy()
        expect(tasks.length).toBe(1)
        expect(tasks[0].state).toBe('processed')
      })
  }, 5000)
      ...
      error: {
        apply: {
          function: (item) => {
            ...
            expect(item.error.message).toBe('apply error')
          }
        }
      }
    }, jobsService)
