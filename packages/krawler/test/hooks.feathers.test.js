...
function createTests (servicePath, feathersHook, options = {}) {
  ...
  it(`reads objects using service ${servicePath}`, async () => {
    ...
    expect(results[0].properties).toBeTruthy()
    expect(results[0].properties.prop0).toBeUndefined()
    expect(results[0].properties.prop1).toBe(0)
    expect(results[1].id).toBe(3)
    expect(results[1].properties).toBeTruthy()
    expect(results[0].properties.prop0).toBeUndefined()
  }, 5000)
  ...
  it(`updates objects using service ${servicePath} and data as option`, async () => {
    ...
    results.forEach(result => {
      expect(result.properties).toBe('value1')
    })
  }, 5000)
  ...
  it(`updates objects using service ${servicePath} and data as item`, async () => {
    ...
    expect(results.length).toBe(3)
    results.forEach(result => {
      expect(result.properties).toBe('value2')
    })
  }, 5000)
  ...
  it(`calls custom method on service ${servicePath}`, async () => {
    ...
    expect(results[0].name).toBe('My custom data')
    expect(results[0].customMethodProperty).toBeTruthy()
    expect(results[0].customMethodProperty).toBe('My custom value')
    expect(results[0].customParamProperty).toBeTruthy()
    expect(results[0].customParamProperty).toBe('My custom param')
  }, 5000)
}
...
describe('krawler:hooks:feathers', () => {
  ...
  it('connect to Feathers', async () => {
    ...
    expect(feathersHook.data.client).toBeTruthy()
    expect(feathersHook.data.client.service('/geojson')).toBeTruthy()
  }, 5000)
  ...
  it('connect to Feathers again', async () => {
    ...
    expect(result).toBe(feathersHook)
  })
  ...
  it('disconnect from Feathers', async () => {
    ...
    expect(feathersHook.data.client).toBeUndefined()
  }, 5000)
