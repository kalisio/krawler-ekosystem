...
export class PrintApp {
  ...
  async selectBaseLayer (test, layerName) {
    const layerSelector = Selector('img').withAttribute('title', layerName)
    await test
  ...
  async selectOverlayLayer (test, layerName) {
    ...
    const layerSelector = Selector('span').withText(layerName).prevSibling()
    await test
  ...
  async print (test, format) {
    const printButton = Selector('.easyPrintSizeMode').withAttribute('title', format)
    await test
